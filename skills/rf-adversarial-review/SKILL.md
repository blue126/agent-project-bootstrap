...
description: 对方案、代码、文章、决策或系统进行对抗式审查，主动构造攻击向量，寻找破绽、盲区、漏洞和反例。仅在用户显式调用 $rf-adversarial-review 时使用；普通审查、review 或意见征询不得触发。
version: 0.1.0
...
仅在用户显式调用 `$rf-adversarial-review` 时使用。普通的「审查」、
「review」、「有没有问题」、「需要改吗」或一般意见征询均按常规任务处理，
不得据此启用本 skill。
