...
- Each Tiger mitigation becomes a tracked task with explicit dependencies.
- The launch readiness gate maps to the upstream `execution/launch-playbook/` workflow when that workflow is available.
- Post-launch monitoring can use the upstream `execution/activation-funnel/` and `execution/north-star-metric/` workflows when available.
- If anything goes wrong despite the mitigations, run a blameless post-mortem.
- The CS pre-training (R4 / R17) can use the upstream `execution/customer-feedback-triage/` workflow when available.
- Comms plan (R12) can use the upstream `execution/launch-playbook/` and `execution/release-notes/` workflows when available.
